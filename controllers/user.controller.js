...
const image = require('../config/config').image
...
UserController.uploadUserProduct = function (req, res) {
  ...
  upload(req, res, (err) => {
    ...
    if (err) return handleResponse(req, res, 500, null, 'error while uploading')
    else {
      if (req.file == undefined) {
        return handleResponse(req, res, 403, null, 'no file selected')
      } else {
        ...
        if (req.body.productname === '' || req.body.price === '') {
          ...
          return handleResponse(req, res, 403, null, 'field(s) cannot be empty')
        }
        ...
        User.uploadproduct(
          ...
          function (err) {
            if (err) return handleResponse(req, res, 500)
